...
function classifyError(status, body) {
  ...
  if (status === 503 || status >= 500) return "provider_5xx";
  if (status === 404 || /no longer available to new users|does not exist|model_not_found|deprecated/i.test(text)) return "model_unavailable";
  if (/timeout|abort/i.test(text)) return "timeout";
  return "provider_error";
}

function isRetryable(errorClass) {
  return errorClass === "rate_limit" || errorClass === "provider_5xx" || errorClass === "timeout";
}
...
async function geminiAnalyze({ imageDataUrl, context, timeoutMs }) {
  ...
  for (const model of geminiModels()) {
    try {
      ...
      if (!response.ok) {
        ...
        if (isRetryable(last.errorClass)) retryableSeen = true;
  ...
  if (retryableSeen) last.errorClass = "provider_5xx";
  return last;
}
...
async function groqAnalyze({ imageDataUrl, context, timeoutMs }) {
  ...
  if (retryableSeen) last.errorClass = "rate_limit";
  return last;
}
...
module.exports = { adapters, providerOrder, extractJson, geminiKey, groqKey, isRetryable };
