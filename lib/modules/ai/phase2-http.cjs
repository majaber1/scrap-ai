...
const { explainRange } = require("../pricing/signals.cjs");
const { scoreBuyer } = require("../matching/engine.cjs");
...
async function handlePhase2(req, res, ctx, parts) {
  ...
  if (parts[0] === "ai" && parts[1] === "drafts" && parts[2] && req.method === "POST" && !parts[3]) return patchDraft(req, res, ctx, parts[2]);
  ...
  if (parts[0] === "pricing" && parts[1] === "signals" && req.method === "GET") return pricing(req, res, ctx);
  if (parts[0] === "price-signals" && req.method === "GET") return pricing(req, res, ctx);
  if (parts[0] === "matching" && parts[1] && req.method === "GET") return matching(req, res, ctx, parts[1]);
  if (parts[0] === "match" && parts[1] && req.method === "GET") return matching(req, res, ctx, parts[1]);
  return false;
}
...
async function getDraft(req, res, ctx, id) {
  ...
  const price = await priceForMaterial(draft.rows[0].material_id);
  return json(res, 200, {
    draft: draft.rows[0],
...
async function rejectDraft(req, res, ctx, id) {
  if (!requirePermission(ctx, res, "listing.manage_own")) return;
  const result = await pool.query(
    `UPDATE ai_listing_drafts SET status='REJECTED', updated_at=now() WHERE id=$1 AND organization_id=$2 AND status NOT IN ('PUBLISHED') RETURNING *`,
    [id, ctx.organizationId]
  );
  if (!result.rows[0]) return json(res, 404, { error: "draft_not_found" });
  await pool.query(
    `INSERT INTO ai_feedback_events(organization_id, analysis_id, draft_id, field_name, ai_value, human_value, source)
     VALUES ($1,$2,$3,'status','REVIEW_REQUIRED','REJECTED','SELLER')`,
    [ctx.organizationId, result.rows[0].analysis_id, id]
  );
...
async function confirmDraft(req, res, ctx, id) {
  ...
  try {
    ...
    const city = String(req.body?.city || draft.rows[0].city || "").trim();
    ...
    const buyers = await client.query(
      `SELECT mr.organization_id
       ...
       WHERE mr.role_code='BUYER' AND mr.organization_id <> $1
    ...
    for (const buyer of buyers.rows) {
      const prior = await client.query(
        `SELECT 1 FROM offers f JOIN scrap_listings l ON l.id=f.listing_id
         WHERE f.buyer_org_id=$1 AND l.material=$2 LIMIT 1`,
        [buyer.organization_id, listing.rows[0].material]
      );
      const scored = scoreBuyer({ hasBuyerRole: true, sameCity: false, sameMaterialOffer: Boolean(prior.rows[0]) });
      await client.query(
        `INSERT INTO buyer_matching_scores(listing_id, buyer_org_id, score, matching_factors) VALUES ($1,$2,$3,$4)`,
        [listing.rows[0].id, buyer.organization_id, scored.score, JSON.stringify(scored.factors)]
      );
    }
    await client.query("COMMIT");
...
async function priceForMaterial(materialId) {
  if (!materialId) return explainRange([]);
  const result = await pool.query(
    `SELECT material_id, region, price_min, price_max, currency, source_type, confidence, observed_at
     FROM market_price_signals WHERE material_id=$1 ORDER BY observed_at DESC LIMIT 20`,
    [materialId]
  );
  return explainRange(result.rows);
}

async function pricing(req, res, ctx) {
  if (!requirePermission(ctx, res, "material.read")) return;
  const materialId = String(queryValue(req, "materialId") || "");
  const explained = await priceForMaterial(materialId || null);
  return json(res, 200, explained);
}

async function matching(req, res, ctx, listingId) {
  if (!requirePermission(ctx, res, "listing.read")) return;
  const listing = await pool.query(`SELECT id FROM scrap_listings WHERE id=$1 AND seller_org_id=$2`, [listingId, ctx.organizationId]);
  if (!listing.rows[0]) return json(res, 404, { error: "listing_not_found" });
  const scores = await pool.query(
    `SELECT id, listing_id, buyer_org_id, score, matching_factors, created_at
     FROM buyer_matching_scores WHERE listing_id=$1 ORDER BY score DESC LIMIT 20`,
    [listingId]
  );
  return json(res, 200, { matches: scores.rows, notice: scores.rows.length ? null : "no_buyer_candidates" });
}
