...
function partsOf(req) {
  ...
  let parts = fromUrl.length >= fromQuery.length ? fromUrl : fromQuery;
  if (url.includes("/v2/ai/drafts") && (parts[0] !== "ai" || parts[1] !== "drafts")) {
    const rest = parts[0] === "ai" ? parts.slice(1) : parts;
    const after = rest[0] === "drafts" ? rest.slice(1) : rest;
    parts = ["ai", "drafts", ...after];
  }
  const draftId = req.query?.id;
  if (url.includes("/v2/ai/drafts") && draftId && !Array.isArray(draftId)) {
    const id = String(draftId);
    const queryAction = String(Array.isArray(req.query?.action) ? req.query.action[0] : req.query?.action || "");
    const action = queryAction === "confirm" || queryAction === "reject" ? queryAction
      : parts.includes("confirm") || url.includes("/confirm") ? "confirm"
      : parts.includes("reject") || url.includes("/reject") ? "reject"
      : "";
    if (!parts.includes(id) || (action && !parts.includes(action))) {
      parts = ["ai", "drafts", id, ...(action ? [action] : [])];
    }
  }
